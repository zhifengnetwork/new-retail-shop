                ...
                <div v-show="sizeBox" class="sku-box-inner" :class="{'box-active':sizeBox}">
                    ...
                    <div class="sku-box-cont">
                        <div class="box-list mb-30" v-for="(list,index) in this.newSpecAttr" :key="index"> 
                            ...
                            <ul class="-list-info">
                                <li 
                                  v-for="(item,key) in list.res" 
                                  :key="key" 
                                  class="-info-a" 
                                  :class="{notClick:item.notClick,active:item.isActiveC}" 
                                  :attr-id="item.attr_id" 
                                  @click="tabInfoChange(index,key,item.attr_id,$event)">
                                    {{item.attr_name}}
                                </li>
                                <!-- :class="[item.isShow ? '' : 'no-sku',sel[index] == key?'sku-active':'']" -->
...
export default {
    ...
    data(){
        return {
            ...
            goodsSkuData:[
              ...
              //       "price": "2243.00",
              //       "stock": "19",
              //       "sku_attr1": "2231,3323,5523"
              //   },
              //   {
              //       "sku_id": "20",
              //       "price": "300.00",
              //       "stock": "29",
              //       "sku_attr1": "2243,3323,5523"
              //   },
              //   {
              //       "sku_id": "21",
              //       "price": "300.00",
              //       "stock": "10",
              //       "sku_attr1": "2231,3354,5523"
              //   },
              //   {
              //       "sku_id": "22",
              //       "price": "300.00",
              //       "stock": "0",
              //       "sku_attr1": "2243,3354,5523"
              //   },
              //   {
              //       "sku_id": "0",
              //       "price": "500.00",
              //       "stock": "48",
              //       "sku_attr1": "2231,3398,5523"
              //   },
              //   {
              //       "sku_id": "0",
              //       "price": "500.00",
              //       "stock": "0",
              //       "sku_attr1": "2243,3398,5523"
              //   }
            ],
            SKUResult:{},        //存放要和选中的值进行匹配的数据
            selectSku:[],       //保存选中的值
            selSkuId:[],
            sel:[],
            // stars:[],
    ...
    tabInfoChange(index, cindex, cid, e) {
      ...
      let orderInfo = _that.newSpecAttr;                      /*所有规格*/
      let orderInfoChild = _that.newSpecAttr[index].res;    /*当前点击的规格的所有子属性内容*/                                         
      if (orderInfoChild[cindex].notClick != true) {    //选中自己，兄弟节点取消选中
      ...
      //已经选择的节点
      let haveChangedId = [];
      for (let i = 0; i < _that.newSpecAttr.length; i++) {
        for (let j = 0; j < _that.newSpecAttr[i].res.length; j++) {
          if (_that.newSpecAttr[i].res[j].isActiveC == true) {
            haveChangedId.push(_that.newSpecAttr[i].res[j].id);
          }
        }
      }
      if (haveChangedId.length) {
        ...
        var prices = _that.SKUResult[haveChangedId.join(";")].prices;
        ...
        for (let i = 0; i < _that.newSpecAttr.length; i++) {
          for (let j = 0; j < _that.newSpecAttr[i].res.length; j++) {
            if (_that.newSpecAttr[index].res[cindex].id != _that.newSpecAttr[i].res[j].id) {
              daiceshi.push({
                index: i,
                cindex: j,
                id: _that.newSpecAttr[i].res[j].id
              });
              daiceshiId.push(_that.newSpecAttr[i].res[j].id);
            }
          }
        }
        ...
        for (let i = 0; i < daiceshi.length; i++) {
          ...
          let siblingsId = "";
          for (let m = 0; m < _that.newSpecAttr[daiceshi[i].index].res.length; m++) {
            if (_that.newSpecAttr[daiceshi[i].index].res[m].isActiveC == true) {
              siblingsId = _that.newSpecAttr[daiceshi[i].index].res[m].id;
            }
          }
          if (siblingsId != "") {
            for (let j = 0; j < len; j++) {
          ...
          testAttrIds = testAttrIds.concat(
            _that.newSpecAttr[daiceshi[i].index].res[daiceshi[i].cindex].id
          );
          testAttrIds.sort(function(value1, value2) {
            return parseInt(value1) - parseInt(value2);
          });
          if (!_that.SKUResult[testAttrIds.join(";")]) {
            _that.newSpecAttr[daiceshi[i].index].res[
              daiceshi[i].cindex
            ].notClick = true;
            _that.newSpecAttr[daiceshi[i].index].res[
              daiceshi[i].cindex
        ...
        for (let i = 0; i < _that.newSpecAttr.length; i++) {
          for (let j = 0; j < _that.newSpecAttr[i].res.length; j++) {
            if (_that.SKUResult[_that.newSpecAttr[i].res[j].id]) {
              _that.newSpecAttr[i].res[j].notClick = false;
            } else {
              _that.newSpecAttr[i].res[j].notClick = true;
              _that.newSpecAttr[i].res[j].isActiveC = false;
            }
          }
        }
      }
    ...
    queryDGoodsById() {
      ...
      for (let i = 0; i < _that.newSpecAttr.length; i++) {
        for (let j = 0; j < _that.newSpecAttr[i].res.length; j++) {
          if (this.SKUResult[_that.newSpecAttr[i].res[j].id] == null) {
            _that.newSpecAttr[i].res[j].notClick = true;
          }
        }
      }
    ...
    initSKU() {     //初始化得到结果集
      var i,j,_that=this,
        skuKeys = _that.getObjKeys(_that.goodsSkuData);
        console.log(_that.goodsSkuData)
      for (i = 0; i < skuKeys.length; i++) {
        var skuKey = skuKeys[i]; //一条SKU信息key
        var sku = _that.goodsSkuData[skuKey]; //一条SKU信息value
        ...
        var combArr = _that.combInArray(skuKeyAttrs);
        for (j = 0; j < combArr.length; j++) {
          _that.add2SKUResult(combArr[j], sku);
        }
 
        //结果集接放入SKUResult
        _that.SKUResult[skuKeyAttrs.join(";")] = {
          count: sku.count,
          prices: [sku.price]
        };
      }
            ...
            .then((res)=>{
                ...
                if(list.status == 200){
                    ...
                    for(var i in _that.newSpecAttr){
                        for(var j in _that.newSpecAttr.res){
                          _that.newSpecAttr.res[j].isActiveC =false
                          _that.newSpecAttr.res[j].notClick =false
                        }
                    }
                    _that.isCollect=this.goodsData.collection
                    console.log(_that.newSpecAttr)
                    _that.queryDGoodsById()     //初始化
                }else{
